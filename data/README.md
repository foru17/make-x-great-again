...
截至 2026-09-01，线上黑名单已累积到 223,670 条。AI 自动判定进入公榜的账号中
出现了会影响正常浏览的误伤，因此本次按收录来源做了一次可审计清理：

- 将全部 21,827 条 `published_tier='ai'` 记录移出当前黑名单；
- 保留直接命中关键词规则的记录和既有人工决定；
- 白名单清理前后均为 2,337 条，没有修改；
- 关闭 AI 直接上榜，后续 AI 判定只进入待审队列，只有独立规则命中或人工决定才会发布。

清理完成时，当前黑名单为 201,843 条。历史 `data-mirror` commit 继续保留作为审计
记录；实时 API、R2 产物和本分支最新快照反映清理后的当前名单。
