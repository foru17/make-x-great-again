...
| 2026-08-02 | `2026-08-02-list-identity-dedup.sql` | ✅ local seed/migrate/idempotency/rollback verified; ⏳ deploy Worker fix, confirm budget alert, then apply | production preview: 97 exact whitelist-add collateral rows + 65 remaining active NULL-id rows with a uid twin + 16 excess pure-NULL rows; adds active NULL-handle uniqueness guard | Removes only audit-proven whitelist collateral and handle-only duplicates; deliberately preserves blacklist rows with different non-null X ids. Companion rollback restores status/source/published_at exactly. |
