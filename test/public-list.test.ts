...
import { createBloomFilter, add, test as bloomTest, serialize, deserialize } from "../src/public-list/bloom.ts";
import { generatePublicList, hashBucket } from "../src/public-list/generate.ts";
import { Meta, PublicEntry, ShardManifest, BloomIndex } from "../src/public-list/schema.ts";
...
test("generatePublicList: filters only human_confirmed spam/porn_bot", () => {
  ...
  const lines = [
    { userId: "1", handle: "a", signalsHash: "h1", model: "m", reviewStatus: "human_confirmed", verdict: { label: "spam", confidence: 0.9, reasons: ["r1"] }, createdAt: "2026-01-01T00:00:00.000Z" },
    { userId: "2", handle: "b", signalsHash: "h2", model: "m", reviewStatus: "auto_pending_review", verdict: { label: "spam", confidence: 0.9, reasons: ["r1"] }, createdAt: "2026-01-01T00:00:00.000Z" },
    { userId: "3", handle: "c", signalsHash: "h3", model: "m", reviewStatus: "human_confirmed", verdict: { label: "legit", confidence: 0.9, reasons: ["r1"] }, createdAt: "2026-01-01T00:00:00.000Z" },
    { userId: "4", handle: "d", signalsHash: "h4", model: "m", reviewStatus: "human_rejected", verdict: { label: "spam", confidence: 0.9, reasons: ["r1"] }, createdAt: "2026-01-01T00:00:00.000Z" },
  ];
...
test("generatePublicList: dedupes by userId, latest wins", () => {
  ...
  const lines = [
    { userId: "42", handle: "old", signalsHash: "h1", model: "m", reviewStatus: "human_confirmed", verdict: { label: "spam", confidence: 0.5, reasons: ["old"] }, createdAt: "2026-01-01T00:00:00.000Z" },
    { userId: "42", handle: "new", signalsHash: "h2", model: "m", reviewStatus: "human_confirmed", verdict: { label: "spam", confidence: 0.9, reasons: ["new"] }, createdAt: "2026-01-02T00:00:00.000Z" },
  ];
  writeFileSync(dbPath, lines.map((l) => JSON.stringify(l)).join("\n"), "utf8");
  const result = generatePublicList({
    sourcePath: dbPath,
  ...
  const shard = JSON.parse(readFileSync(join(tmp, "out-dedup", "data", "shards", hashBucket("42", 256) + ".json"), "utf8"));
...
test("generatePublicList: meta and index schemas validate", () => {
  ...
  const lines = [
    { userId: "100", handle: "a", signalsHash: "h1", model: "m", reviewStatus: "human_confirmed", verdict: { label: "spam", confidence: 0.9, reasons: ["r1"] }, createdAt: "2026-01-01T00:00:00.000Z" },
  ];
  writeFileSync(dbPath, lines.map((l) => JSON.stringify(l)).join("\n"), "utf8");
