...
const isSafariBuild = process.env.MXGA_SAFARI === "1";
...
export default defineConfig({
  ...
  outDirTemplate: isSafariBuild
    ? "{{browser}}-mv{{manifestVersion}}-safari"
  ...
  manifest: ({ mode }) => {
    return {
      ...
      options_ui: isSafariBuild ? {} : { open_in_tab: true },
    };
  },
  hooks: {
    "build:manifestGenerated": (wxt, manifest) => {
      ...
      if (manifest.options_ui) {
        delete (manifest.options_ui as { open_in_tab?: boolean }).open_in_tab;
      }

      for (const script of manifest.content_scripts ?? []) {
        delete (script as { world?: string }).world;
      }
    },
  },
});
